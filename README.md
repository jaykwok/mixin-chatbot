...
**工具进度卡片**：agent 每调一次工具，群里发一条 `🔧 工具: 入参` 进度（如 `🔧 bash: ls -la ./data`、`🔧 read: ./data/notes.txt`），让你看到它在干嘛、好决定要不要干预。
...
适合无公网 IP 的云电脑：bot 跑在云电脑 :1011，`cloudflared` 把 `im-bot.example.com` 经 Cloudflare 隧道接到本机。
...
4. IM 平台回调填：`https://im-bot.example.com/webhook/<secret>`（secret 来自 deploy 输出）。

> 公网 1011 无需开放（`cloudflared` 本地连）。Cloudflare WAF（平台 IP 白名单）在服务端配置。隧道/域名已在服务端建好（`im-bot.example.com` → 隧道 → 本机 :1011）。
