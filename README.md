...
| 监控 | 体检、机器人日志、隧道日志 | 查看故障与修复建议；搜索日志、筛选级别、按任务编号提取排查记录 |
...
Cloudflared 日志开关为“关闭 / 开启”，默认关闭；开启后写入 `logs/cloudflared.log` 并自动轮转，在 **监控 → 隧道日志** 搜索请求路径、错误或 `cfRay`。界面时间转换为本机时间，原始文件为 UTC。切换会重启正在运行的本项目隧道，已停止时保持停止。详见[隧道日志](docs/operations.md#隧道日志)。

Cloudflared 连接模式提供“自动（默认）/ HTTP2（TCP）/ QUIC（UDP）”。自动优先 QUIC，无法建立 UDP 连接时回退 HTTP2；UDP 不稳定时可尝试 HTTP2。切换会重启正在运行的本项目隧道，停止时下次启动生效。详见[隧道连接模式](docs/operations.md#隧道连接模式)。
...
部署入口运行当前代码。选中“升级”后会在后台拉取 `origin/main`，显示本次检查的当前提交、目标提交和待应用列表；检查完成后按 Enter 确认，等待期间仍可切换操作或页面。检查失败时按 `r` 重试；重新选中升级也会检查远端。预览只更新远端引用，不改本地代码或重启服务，确认执行后才进入升级流程，并再次核对远端。新机器需先安装 Bun 及平台部署所需的环境，之后即可从“系统 → 服务部署”进入部署。旧账本格式需要在升级前完成转换，启动或重启不会自动迁移；模型配置重建见[运维手册](docs/operations.md#重新配置模型)。
...
向导内提供 Alist + Cloudflare 子域名示例：上传地址填写到 Alist 挂载目录，例如 `127.0.0.1:5244/dav/relay`，此处挂载目录为 `relay`。公开下载项只填 `files.example.com` 即可推导为 `https://files.example.com/d/relay/`；也可填写完整下载目录地址。`/dav/` 是 WebDAV 入口，`/d/` 是下载入口，后面的 `relay` 换成实际挂载路径或其子目录。Cloudflare 路由及账号权限见[外链配置示例](docs/operations.md#alist-与-cloudflare-子域名示例)。
