...
`ops.ps1 doctor` / `ops.sh doctor` 在检测到 `relay.json` 时会多出三行：配置本身是否有效、WebDAV 上传端点是否可达且凭证有效（`PROPFIND Depth:0`）、公开下载基址是否有应答。文件不存在时这三行不出现——特性可选，没配不是问题。Linux 侧的连通性探测需要 `jq`，缺失时只报告「已启用」并跳过探测。
...
- **链接并进模型那条回复，不单独发。** 平台按条限流，「工具发链接 + 模型发说明」一次发送就吃掉两条配额。但也不能反过来让模型自己把链接写进回复里——URL 带 uuid 和百分号编码，逐字复述正是模型最容易出错的地方，错一个字符文件就下不到。所以工具把这段话交给运行时，由代码在发送前原样拼到回复末尾，模型只负责说人话；工具结果里也明确要求它不要重复粘贴链接。若本轮回复最终没能发出去（模型报错、`/stop`、发送失败），运行时会把这段话单独补发一条——文件此时已经躺在外链后端上了，链接跟着失败一起消失的话谁都拿不到它。
