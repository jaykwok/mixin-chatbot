...
各页在进入时读取所需数据。总览的服务状态、今日用量、磁盘占用和版本信息会分别显示；切回已查看的页面时保留已有内容并更新。按 `r` 或从空格菜单刷新均在后台进行，慢查询期间仍可切页。系统页菜单立即显示，版本读取期间仅升级需要等待。
...
界面读取宿主机上的部署数据，维护操作复用 `ops.sh` / `ops.ps1`。“监控 → 体检”复用 `doctor --json`（Windows 为 `doctor -Json`）的逐项结果；等待体检期间仍可进入修复确认。有失败项时，JSON 接口返回非零退出码。
