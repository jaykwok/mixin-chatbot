...
describe("project system prompt", () => {
  test("does not promise an unavailable relay or installed parser", () => {
    const prompt = buildChatContext({ tempDir: "agents/temp/user", relayEnabled: false });
  ...
  test("enables only configured delivery capability", () => {
    expect(buildChatContext({ tempDir: "agents/temp/user", relayEnabled: true })).toContain("已配置的外链服务");
  });
  ...
  test("keeps user-specific location after the stable role and capabilities", () => {
    const a = buildChatContext({ tempDir: "agents/temp/user-a", relayEnabled: false });
    const b = buildChatContext({ tempDir: "agents/temp/user-b", relayEnabled: false });
