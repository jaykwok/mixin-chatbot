import { loadTunnelLogging, type TunnelLogging } from "../data.ts";
...
export class SettingsView implements View {
  ...
  activity(): string | null {
    if (this.editingRuntime || this.selected === 2) return this.runtime.activity();
  ...
  invalidate(): void { this.tunnel.invalidate(); this.runtime.invalidate(); }
  onLeave(): boolean {
    const pending = this.tunnel.onLeave();
    const runtimePending = this.runtime.onLeave();
    return pending || runtimePending;
  }

  refresh(app: AppApi): Promise<void> {
    if (this.editingRuntime || this.selected === 2) return this.runtime.load(app, true);
  ...
  async onKey(key: { name: string }, app: AppApi): Promise<boolean> {
    ...
    const selected = moveSelection(key.name, this.selected, 3);
    if (selected !== null) {
      this.selected = selected;
      if (selected === 1) void this.tunnel.load(app); else this.tunnel.onLeave();
      if (selected === 2) void this.runtime.load(app); else this.runtime.onLeave();
      return true;
    }
    const action = key.name === "enter" ? ["e", "l", "a"][this.selected] : key.name;
    if (action === "v") { app.go("tunnel-logs"); return true; }
    if (action === "a") {
      this.selected = 2; this.editingRuntime = true; this.tunnel.onLeave();
    ...
    this.selected = 1; this.runtime.onLeave();
