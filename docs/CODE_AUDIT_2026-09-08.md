...
审计基线为 `4d51f630c436dfe98b488fe1b1b092fc13447c85`，Bun 1.4.0、Windows、Pi 0.85.0。目标版本为 **Pi 0.85.1**。范围包括 `src/`、`scripts/`、`tests/`、依赖和锁文件、Docker、README、系统提示词、工具描述及 Pi SDK 接入。用户已授权按照本文全量整改，允许为降低长期维护成本断兼容。旧远端外链已确认删除，不引入旧账本转换工具。本文保留原始发现、首轮验证及复核修复记录。
...
| R08 / 简化 | 移除运行时旧外链 JSONL 导入表和解析器、旧 key 重命名接口、平铺对象删除分支；账本状态和后端命名空间必填。旧外链已确认清空，不加入一次性转换工具。 | 用当前 SQLite 数据覆盖大账本和损坏保护；新增切换后端保持独立对象记录，拒绝推测不合规对象的删除目标。 |
