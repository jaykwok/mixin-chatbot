...
从 Pi 0.85.1 升级时，配置迁移和版本登记由[升级事务](data-migrations.md)完成；Windows 首次过渡需要按该文档先停机。保温默认关闭，历史无需格式转换。迁移程序随代码发布在 `scripts/migrations/`，升级不依赖 `tmp/` 中的工具。
