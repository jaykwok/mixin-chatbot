...
`update` 要求已跟踪文件没有本地改动，代码已最新也会检查数据版本。Windows 和 Docker 都先确认旧实例停止，再切换工作树、安装依赖或构建镜像；下载目标提交和只读检查可在停机前完成。两个平台的升级都沿用已保存的端口、入口模式、群数据根、域名和 AI 配置，停机后不再询问配置；修改这些配置请使用 `deploy`。Docker 需要确认的迁移决策仍在构建新镜像后询问。Docker 在停机前确认未托管 Cloudflared 的归属，并在缺少连接器和可用 token 时先收集 token，交给部署脚本在验证实例就绪后启动一次；缺少 `data/config/models.json` 时拒绝升级，需先通过 `deploy` 完成配置。需要迁移时先备份再转换，随后使用只验证实例检查，提交后恢复原运行状态。同版本且标记配对时跳过数据迁移和数据库备份，仍做配置、schema 和新实例校验。提交前失败恢复数据、原提交和部署；提交后启动失败保留新代码与数据，检查日志后重试。首次升级和中断续做参见[数据版本与升级事务](data-migrations.md)。
...
当前版本要求待补发账本 schema 2。更早格式不在现有迁移的支持范围内；遇到格式错误时保持停机，保留数据库与 WAL，按[旧账本说明](data-migrations.md#未支持的旧账本)处理。不要依据旧错误提示寻找未随仓库发布的 tmp 脚本，也不要删表来绕过校验。
