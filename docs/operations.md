...
`update` 要求已跟踪文件没有本地改动，代码已最新也会检查数据版本。Windows 和 Docker 都先确认旧实例停止，再切换工作树、安装依赖或构建镜像；下载目标提交和只读检查可在停机前完成。Docker 在停机前按当前入口模式确认未托管 Cloudflared 的归属，并在缺少连接器和可用 token 时先收集 token，交给部署脚本在验证实例就绪后启动一次；升级中改选入口模式时，由部署脚本在其预检阶段询问。需要迁移时先备份再转换，随后使用只验证实例检查，提交后恢复原运行状态。同版本且标记配对时跳过数据迁移和数据库备份，仍做配置、schema 和新实例校验。提交前失败恢复数据、原提交和部署；提交后启动失败保留新代码与数据，检查日志后重试。首次升级和中断续做参见[数据版本与升级事务](data-migrations.md)。
