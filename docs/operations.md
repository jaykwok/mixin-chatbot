...
`update` 要求已跟踪文件没有本地改动，代码已最新也会检查并迁移数据。升级先收集迁移决策，停机后备份并迁移，使用只验证实例检查，再提交数据版本并恢复原运行状态。提交前失败恢复数据、原提交和部署；提交后启动失败保留新代码与数据，检查日志后重试。Windows 在改工作树与依赖之前停止实例，Linux 已运行容器与主机源码隔离。首次升级和中断续做参见[数据版本与升级事务](data-migrations.md)。
