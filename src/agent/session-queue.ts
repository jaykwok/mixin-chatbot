...
export class SessionQueue {
  private pending: Job[] = [];
  private current?: { controller: AbortController; done: Promise<void>; invalidate?: () => void };
  private closed = false;
  phase = "空闲";
  get waiting(): number { return this.pending.length; }
  get busy(): boolean { return !!this.current; }

  enqueue(task: Job["task"], invalidate?: () => void): Promise<void> {
    if (this.closed || this.pending.length >= 8) {
      invalidate?.();
      return Promise.reject(this.closed ? abortError("会话已关闭") : new Error("本会话已有 8 条消息排队，请稍后重发"));
    }
    const result = new Promise<void>((resolve, reject) => this.pending.push({ task, resolve, reject, invalidate }));
    this.pump();
    return result;
  }
  ...
  cancel(): Promise<void> {
    ...
    for (const job of this.pending.splice(0)) { job.invalidate?.(); job.reject(reason); }
  ...
  private pump(): void {
    ...
    this.current = { controller, done, invalidate: job.invalidate };
    this.phase = "准备中";
